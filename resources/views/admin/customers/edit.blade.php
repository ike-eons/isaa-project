    ...
    <div class="row">
        <div class="col-md-8 mx-auto">
            <div class="tile">
                ...
                <form action="{{ route('admin.customers.edit',$targetCustomer->id) }}" method="POST" role="form" enctype="multipart/form-data">
                    @csrf
                    <div class="tile-body">
                        
                        <div class="form-group">
                            <label class="control-label" for="firstname">First Name <span class="m-l-5 text-danger"> *</span></label>
                            <input class="form-control @error('firstname') is-invalid @enderror" type="text" name="firstname" id="firstname" value="{{ old('firstname',$targetCustomer->firstname) }}"/>
                            <input type="hidden" name="id" value="{{ $targetCustomer->id }}">
                            <span class="text-danger">@error('firstname') {{ $message }} @enderror</span>
                        </div>

                        <div class="form-group">
                            <label class="control-label" for="lastname">Last Name <span class="m-l-5 text-danger"> *</span></label>
                            <input class="form-control @error('lastname') is-invalid @enderror" type="text" name="lastname" id="firstname" value="{{ old('lastname',$targetCustomer->lastname) }}"/>
                            <span class="text-danger">@error('lastname') {{ $message }} @enderror</span>
                        </div>

                        <div class="form-group">
                            <label class="control-label" for="phone">Phone <span class="m-l-5 text-danger"> *</span></label>
                            <input class="form-control @error('phone') is-invalid @enderror" type="text" name="phone" id="phone" value="{{ old('phone',$targetCustomer->phone) }}"/>
                            <span class="text-danger">@error('phone') {{ $message }} @enderror</span>
                        </div>

                        <div class="form-group">
                            <label class="control-label" for="shop_name">Shop Name <span class="m-l-5 text-danger"> *</span></label>
                            <input class="form-control @error('shop_name') is-invalid @enderror" type="text" name="shop_name" id="shop_name" value="{{ old('shop_name',$targetCustomer->shop_name) }}"/>
                        ...
                        <textarea class="form-control" name="shop_address" id="shop_address" cols="30" rows="5">{{$targetCustomer->shop_address}}</textarea>
